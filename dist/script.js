    ...
    value: function bindTriggers(officer, counter, items) {
      officer.querySelector('.plus').addEventListener('click', function () {
        ...
        if (counter !== items.length - 2) {
          ...
          // якщо ми дойшли до останньої карточки яку треба показати, ми її показазуємо і після цього видаляєм останній блок
          items[counter].style.display = 'flex';
          items[counter].classList.add('animated', 'fadeIn');
    ...
    value: function init() {
      ...
      this.forms.forEach(function (item) {
        item.addEventListener('submit', function (e) {
          ...
          item.parentNode.appendChild(statusMessage); // показуємо користувачц що йде загрузка

          statusMessage.textContent = _this.message.loading;
          var formData = new FormData(item);

          _this.postData(_this.path, formData) // обробляєм запит
          .then(function (res) {
            console.log(res);
  ...
  _createClass(VideoPlayer, [{
    ...
    value: function onPlayerStateChange(state) {
      try {
        ...
        var blockedElem = this.activeBtn.closest('.module__video-item').nextElementSibling; // створюєм змінну в яку помістимо svg іконку play
        // cloneNode(true) - глибоке копіювання

        var playBtn = this.activeBtn.querySelector('svg').cloneNode(true); // якщо відео вже переглянуто (0 - з документації)
      ...
      this.prev.addEventListener('click', function () {
        ...
        for (var i = _this2.slides.length - 1; i > 0; i--) {
          if (_this2.slides[i].tagName !== "BUTTON") {
            ...
            var active = _this2.slides[i]; // ставимо його на першу позицію
...
var Slider = // container - контейнер для слайдера
